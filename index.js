...
const venenoTrafficBot = async id => {
	...
	try {
		...
		.useragent(ua)
		//.goto('https://www.bing.com/search?q=foodblog.click')
        //.type('input[name=q]', keywordlist[Math.floor(Math.random() * keywordlist.length)])
        // .type('input[name=q]', 'foodblog.click')
		// .click('input[name=go]')
